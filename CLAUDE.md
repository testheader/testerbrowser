...
- **Session snapshot export/import, and its best-effort React state restore** — "Export snapshot…" / "Import snapshot…" (tab context menu, `sessionManager.ts`) capture cookies, localStorage, sessionStorage, IndexedDB, form field values, scroll position and `history.state`, per-frame (main frame + same-page iframes, via `WebFrameMain.framesInSubtree`). Snapshot files are versioned (`version: 2`); v1 files (single implicit frame, storage inline) still import via a compatibility path in `restoreSnapshot`.
  - Every session gets a minimal React DevTools hook stub injected at page-start (`snapshotScripts.ts`'s `REACT_HOOK_STUB_SCRIPT`, installed once in `createSession` via CDP `Page.addScriptToEvaluateOnNewDocument`). Without it `window.__REACT_DEVTOOLS_GLOBAL_HOOK__` never exists — that global is normally injected by the DevTools *browser extension*, which TesterBrowser doesn't install — so React never registers with it and the reactState capture below silently finds nothing.
  - `COLLECT_FRAME_SCRIPT` walks the fiber tree through that hook and records each component's state (class `instance.state`, or each hook's `memoizedState` for function components), keyed by a "Component > Component" path.
  - On import, `buildRestoreFrameScript` re-walks the freshly-mounted tree after reload, matches nodes by that same path, and writes state back only through APIs the component itself would use: `instance.setState(...)` for class components, and a hook's own `queue.dispatch` (the exact function `useState` returned as its setter) for hooks — but only when `queue.lastRenderedReducer` looks like React's built-in `basicStateReducer`, so a `useReducer` hook's dispatch never gets a raw value fed in as if it were an action. This is undocumented, version-dependent internal fiber layout, not a public API — it can silently restore nothing on a page it can't confidently match, under minification (where `basicStateReducer`'s name may not survive), or on a future React version that changes shape. It's reported as a warning after import (e.g. "reactState: restored N hook/state value(s) across M matched component(s) of T captured"), never a crash or corrupted state.
